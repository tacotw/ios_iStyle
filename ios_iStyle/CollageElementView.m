...
@end
@implementation CollageElementView
...
-(id)init{
    if ((self = [super init])) {
        ...
        _textLabel.backgroundColor = [UIColor colorWithWhite:0. alpha:0.6];
...
- (void)centerScrollViewContents {
    ...
    if (contentsFrame.size.width < boundsSize.width) {
        contentsFrame.origin.x = (boundsSize.width - contentsFrame.size.width) / 1.5f;
    } else {
        contentsFrame.origin.x = 0.0f;
    }
    
    if (contentsFrame.size.height < boundsSize.height) {
        contentsFrame.origin.y = (boundsSize.height - contentsFrame.size.height) / 1.5f;
