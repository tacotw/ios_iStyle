...
- (void)searchWithCluster:(NSArray *)keywords clusterName:(NSString *)clusterName offset:(NSInteger)offset limit:(NSInteger)limit {
    ...
    if (keywords == nil || keywords.count == 0) {
        url = [NSURL URLWithString:[NSString stringWithFormat:@"http://tw.egs.srch.ect.yahoo.com:4080/v2/egs/list/shopping/QueryByClusterName/?cluster=%@&offset=%d&hits=%d", [clusterName stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLHostAllowedCharacterSet]], offset-1, limit]];
        self.searchInterface = SEARCH_INTERFACE_API;
    }
    else {
        NSString *keyword = [keywords componentsJoinedByString:@" "];
        NSMutableString *query = [NSMutableString string];
        [query appendString:[NSString stringWithFormat:@"SELECT * FROM oneecsearch.search (%d,%d) WHERE keyword='%@' and property='shopping'", offset, limit, keyword]];
        if (clusterName != nil) {
